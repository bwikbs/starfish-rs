...
| **E48-M3** | **`cross-fade()`**: `cross-fade(<image> <p>, <image>)` blends two images/gradients by the percentage (paint both into a layer, weight by the fade). | `css`, `style`, `paint` | `cross-fade(linear-gradient(...) 50%, linear-gradient(...))` blends two gradients 50/50 (tested + visual) | ✅ |
