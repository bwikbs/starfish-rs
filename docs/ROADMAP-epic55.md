...
| **E55-M3** | **`<mask>` + basic `<filter>`**: `mask="url(#m)"` clips/fades a shape by the luminance/alpha of the `<mask>`'s content; `filter="url(#f)"` with an `feGaussianBlur` blurs the element. | `paint` | a shape with `mask=url(#m)` is masked by the mask content; `filter` with feGaussianBlur blurs it (tested + visual) | ✅ |
