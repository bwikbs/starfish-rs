...
**Epic 17 complete.** 1084 workspace tests, clippy clean. CSS animations &
transitions rendered as a static frame at a `--at <seconds>` clock: @keyframes +
animation-* + easing + opacity/color/length interpolation (M1), transform-list
interpolation + delay/iteration/direction/fill-mode (M2), and JS-driven
transitions (pre-script snapshot + second cascade) + border-color/border-radius/
box-shadow interpolation (M3). Byte-identical for pages without animations or
transitions.
