...
|-----------|-------|--------|-----------|--------|
| **E53-M1** | **`list-style-image`**: `list-style-image: url(...)` uses the image as the list marker (decoded like a background/`<img>`); falls back to the `list-style-type` marker when the image is absent/none. `list-style` shorthand accepts the image. | `style`, `layout`, `paint` | a `<ul style="list-style-image:url(dot.png)">` paints the image as each item's marker (tested + visual) | ☐ |
| **E53-M2** | **`content: url()` + `content: none`**: `content` on `::before`/`::after` may be a `url(...)` → an image replaced box; `content: none`/`normal` suppresses the pseudo (already partly). A `content` list mixing strings/`url()`/`counter()`/`attr()` renders in order. | `style`, `layout`, `paint` | `::before{content:url(icon.png)}` paints the image before the element; mixed `content:"[" attr(data-x) "]"` renders in order (tested + visual) | ☐ |
