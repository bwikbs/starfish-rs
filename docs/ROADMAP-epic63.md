...
|-----------|-------|--------|-----------|--------|
| **E63-M1** | **`<template>` inert + `.content`**: UA `template { display: none }` so its content is not rendered; JS `template.content` returns a fragment-like wrapper whose `children`/`querySelector` see the template's parsed children. | `style`, `js` | a page with `<template><p>x</p></template>` renders nothing for it; `tmpl.content.querySelector('p')` finds the `<p>` (tested + visual) | ☐ |
