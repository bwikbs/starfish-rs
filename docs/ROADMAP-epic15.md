...
**Epic 15 complete.** 988 workspace tests, clippy clean. Images & media across three
milestones, byte-identical for default raster images: gif/webp/bmp + object-fit/position +
image-rendering (M1), srcset/sizes + `<picture>` responsive selection (M2), and SVG-as-image
(vector, via the inline-SVG painter) + video/audio poster/placeholder + broken-image alt (M3).
