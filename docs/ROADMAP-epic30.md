...
| **E30-M3** | **Syntax-validated initial**: a registration whose `initial-value` doesn't parse against its `syntax` (a small subset: `<length>`/`<color>`/`<number>`/`<percentage>`/`<integer>`/`*`) is dropped, so only well-typed registrations contribute an initial value. (The `inherits: false` non-inheriting-reset behavior is deferred — see non-goals — as it requires threading per-property inheritance through the cascade.) | `css` | an `@property` whose `initial-value` doesn't match its `syntax` is dropped (so `var()` invalidates rather than using a mistyped default); a matching one is kept; `syntax: "*"` needs no initial (tested) | ✅ |

## Non-goals (deferred)

- `inherits: false` non-inheriting reset (a child returning to the initial value
  even when an ancestor set the property): requires threading per-property
  inheritance flags through the cascade's custom-property resolution. Registered
  properties currently always inherit (the common `inherits: true` case).
