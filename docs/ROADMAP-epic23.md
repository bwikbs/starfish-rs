...
**Epic 23 complete.** 1264 workspace tests, clippy clean. HTML entities & parser
robustness across three milestones: comprehensive named + numeric character
references in text and attributes (M1, fixing the `&shy;` gap), table-group +
`optgroup` implied end tags (M2; most auto-close already existed), and
RAWTEXT/RCDATA content modes for script/style/textarea/title + a table-scope fix
for nested tables (M3). Well-formed entity-free pages stay byte-identical.
