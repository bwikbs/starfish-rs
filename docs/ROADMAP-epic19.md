...
**Epic 19 complete.** 1140 workspace tests, clippy clean. JS DOM & web-API
expansion across three milestones, byte-identical for pages not exercising the new
APIs: classList/traversal/insertion gaps (M1), layout geometry via on-demand
mid-script layout (M2), and requestAnimationFrame/MutationObserver/history+location/
matchMedia (M3).
