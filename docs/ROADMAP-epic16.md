...
**Epic 16 complete.** 1042 workspace tests, clippy clean. A second CSS pass
across four milestones, byte-identical for pages not using the new features:
`:is()`/`:where()`/`:has()` + CSS counters (M1), `background-image` url layers +
size/position/repeat + multiple layers (M2), radial/conic gradients + text-shadow
+ outline (M3), and `text-overflow: ellipsis` + `position: sticky` (M4).
