...
`box-sizing`, `min/max`, `calc`, `@media`, overflow clipping. **Grid** and
**flexbox** both support `gap`/`row-gap`/`column-gap` (the shared gap fields landed
with the grid epic; flex consumes them on the main axis and between wrapped lines).
There is no `aspect-ratio` (a box with one auto dimension never derives the
...
|-----------|-------|--------|-----------|--------|
| **E18-M1** | **aspect-ratio** (real work) + **flex gap** (confirm/regression): `aspect-ratio: <w>/<h>` (and `<number>`) — when a block box has a definite size on one axis and `auto` on the other (and no other constraint), derive the other from the ratio, respecting `min/max` (replaced-content CSS aspect-ratio deferred). Flexbox `gap`/`row-gap`/`column-gap` is **already implemented** (shared gap fields, consumed by flex main + cross axis) — M1 just adds regression coverage and fixes the stale "flexbox does NOT" note above. | `style`, `layout` | A box with `aspect-ratio:16/9` + a definite width lays out at the derived height; flex items with `gap:10px` stay 10px apart (regression test) | ✅ |
...
**Epic 18 complete.** 1106 workspace tests, clippy clean. Layout round 3 across
three milestones, byte-identical for pages not using the new features:
`aspect-ratio` + flex-gap confirmation (M1), CSS multi-column (`column-*` +
`column-rule`, greedy balance) (M2), and vertical writing modes (`writing-mode`
vertical-rl/lr + `text-orientation` upright/sideways) (M3).
